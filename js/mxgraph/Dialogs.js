...
var LayersWindow = function (editorUi, x, y, w, h) {
  ...
  mainDiv.style.backgroundColor = (Dialog.backdropColor == 'white') ? '#dcdcdc' : Dialog.backdropColor;
  mainDiv.style.position = 'absolute';
  mainDiv.style.overflow = 'auto';
  mainDiv.style.left = '0px';
  mainDiv.style.right = '0px';
  mainDiv.style.top = '0px';
  mainDiv.style.bottom = (parseInt(tbarHeight) + 7) + 'px';
...
* 新增截图工具界面
* */
let ImageWindow = function (editorUi, x, y, w, h) {
  ...
  ldiv.style.display = 'block';
  ldiv.style.whiteSpace = 'nowrap';
  /*ldiv.style.backgroundColor = '#FFFF00';*/
  if (mxClient.IS_QUIRKS) {
    ldiv.style.filter = 'none';
  }
  ...
  // 打开图片
  let fileInput = document.createElement('input'); // input用于打开文件
  /*fileInput.setAttribute('multiple', 'multiple');*/
  fileInput.setAttribute('type', 'file');
  fileInput.setAttribute('id', 'fileInput');

  // 添加图片
  let addBtn = link.cloneNode(); // 克隆一个<a>结点
  addBtn.innerHTML = '<div id="addBtn" class="geSprite geSprite-plus" style="display:inline-block;"></div>'; // <a>结点中添加图标
  addBtn.setAttribute('title', mxResources.get('addImages'));  // 设置添加按钮的title属性
  ...
  if (Graph.fileSupport) {
      ...
      if (graph.isEnabled()) {
        ...
        try {
          ...
          img.onload = function () {
            ...
            myCanvas.width = img.naturalWidth;  // 根据图片宽高调整canvas宽高
            myCanvas.height = img.naturalHeight;
            ctx = myCanvas.getContext('2d');
            /*console.log('图片原始宽高: ', originWidth, ' x ', originHeight);*/
            ctx.save();
            ctx.clearRect(0, 0, window.innerWidth, window.innerHeight);  // 清空canvas画布
            ctx.scale(scaleX, scaleX);
            ctx.drawImage(img, 0, 0, originWidth, originHeight);  // 将图片加入到canvas中
            ctx.restore();
          };
        } finally {
          graph.model.endUpdate();
    ...
    if (!graph.isEnabled()) {
      addBtn.className = 'geButton mxDisabled';
    }
    ldiv.appendChild(addBtn);  // 将按钮作为底部工具栏的子元素加入
  }
  ...
  * */
  // 截图框按钮
  let cutBtn = link.cloneNode();
  cutBtn.innerHTML = '<div class="geSprite geSprite-shadow" style="display:inline-block;"></div>';
  cutBtn.setAttribute('title', mxResources.get('clipImage'));
  ...
  mainDiv.onscroll = function () {
    /*console.log('mainDiv.scrollLeft / mainDiv.scrollTop', mainDiv.scrollLeft, ' / ', mainDiv.scrollTop);*/
  };

  var croping = false;
  // 截图框按钮
  mxEvent.addListener(cutBtn, 'click', function (evt) {
    if (graph.isEnabled()) {
      graph.model.beginUpdate();
      try {
        /*console.log('截取图片...');*/
        croping = true; // 表示正在截图...
        var clickFlag = false;
        // 获取canvas中图片实际大小，此处显示宽高即为图片原始宽高
        var iCurWidth = originWidth;
        var iCurHeight = originHeight;
        /*console.log('图片当前实际宽高=', iCurWidth, 'x', iCurHeight);*/

        // 可调整截图框
        oRelDiv.innerHTML = '';
        oRelDiv.style.zIndex = 999;
        ...
        scaleX = iCurWidth / iOrigWidth; // 图片宽度缩放比例（当前实际/原始）
        scaleY = iCurHeight / iOrigHeight;  // 图片高度缩放比例（当前实际/原始）*/
        /*console.log('图片横纵（宽度/高度）缩放比=', scaleX, ' / ', scaleY);*/

        // 将oRelDiv插入到myCanvas前，insertBefore() 方法在您指定的已有子节点之前插入新的子节点。此处意为将oRelDiv作为mainDiv的子结点(myCanvas的兄弟结点）插入到myCanvas之前
        myCanvas.parentNode.insertBefore(oRelDiv, myCanvas);

        //设置初始化时截选框坐标与高宽
        var cropW = 120; //截图框默认宽度
        var cropH = 120; //截图框默认高度
        /*console.log('myCanvas.offsetLeft=', myCanvas.offsetLeft);
        console.log('myCanvas.offsetTop=', myCanvas.offsetTop);*/
        var mainDivWidth = window.getComputedStyle(mainDiv, null).getPropertyValue("width");    // div当前宽度
        var mainDivHeight = window.getComputedStyle(mainDiv, null).getPropertyValue("height");  // div当前高度
        var containerWidth = parseInt(mainDivWidth.substring(0, mainDivWidth.indexOf('px')));
        var containerHeight = parseInt(mainDivHeight.substring(0, mainDivHeight.indexOf('px')));

        /*console.log('divWidth:', mainDivWidth);
        console.log('divHeight:', mainDivHeight);
        console.log('containerHeight:', containerWidth);
        console.log('containerHeight:', containerHeight);

        console.log('mainDiv.scrollLeft:', mainDiv.scrollLeft);
        console.log('mainDiv.scrollTop:', mainDiv.scrollTop);*/

        // 设置截选框其实位置为窗口中央
        var posX = containerWidth / 2 - cropW / 2 + mainDiv.scrollLeft;  // 截图框左上角x坐标
        var posY = containerHeight / 2 - cropH / 2 + mainDiv.scrollTop;    // 截图框左上角y坐标
        /*console.log('截选框起始位置posX=', posX);
        console.log('截选框起始位置posY=', posY);*/
        oRelDiv.innerHTML = '<div id="zxxCropBox" style="height:' + cropH + 'px; width:' + cropW + 'px; position:absolute; left:' +
          ...
          '<button id="confirmBtn" style="position:absolute; width:30px; height: 20px; overflow:hidden; right:0; bottom:-30px; background: #4CAF50; cursor:pointer; border: none; border-radius: 5px;color: white; font-size: 12px;" >确认截图</button>' +
          '<input type="hidden" id="cropPosX" value="' + posX + '" style="position:relative; top: -26px; width: 30px"/>' +
          '<input type="hidden" id="cropPosY" value="' + posY + '" style="position:relative; top: -26px; width: 30px"/>' +
          '<input type="hidden" id="cropImageWidth" value="' + cropW + '" style="position:relative; top: -26px; width: 30px"/>' +
          '<input type="hidden" id="cropImageHeight" value="' + cropH + '" style="position:relative; top: -26px; width: 30px"/>' +
          '<div id="cropSize" style="position:absolute; bottom:-30px; left:0; overflow: hidden; border-radius: 5px">' +
          '<label id="imgWidth" style="width: 30px; font-size: 12px; color: #000000; overflow: hidden">' + 120 / scaleX + '</label>' +
          '<label id="imgX" style="width: 6px; font-size: 12px;color: #000000; overflow: hidden"> x </label>' +
          '<label id="imgHeight" style="width: 30px; font-size: 12px; color: #000000; overflow: hidden">' + 120 / scaleX + '</label>';
          '</div>' +
          '</div>';
        ...
        var startDrag = function (point, target, kind) {
          ...
          point.onmousedown = function (event) {
            ...
            /*console.log('鼠标按下时x坐标（params.currentX）=', params.currentX);
            console.log('鼠标按下时y坐标（params.currentY）=', params.currentY);*/
            //防止IE文字选中，有助于拖拽平滑
            ...
            document.onmousemove = function (event) {
              ...
              ID("imgWidth").innerText = parseInt(parseInt(ID("zxxCropBox").style.width) / scaleX);
              ID("imgHeight").innerText = parseInt(parseInt(ID("zxxCropBox").style.height) / scaleX);
              ID('cropSize').style.display = parseInt(ID("zxxCropBox").style.width) < 90 ? 'none' : 'inline';
        ...
        confirmBtn.addEventListener("click", function () {
          let x = document.getElementById("cropPosX").value;  // 实际截选框X坐标
          let finalX = x / scaleX; // 原始尺寸上截选框的X坐标
          let y = document.getElementById("cropPosY").value;
          let finalY = y / scaleX;
          let w = document.getElementById("cropImageWidth").value; // 实际宽度
          let finalW = parseInt(w) / scaleX; // 原始尺寸上截选框的宽度
          let h = document.getElementById("cropImageHeight").value;
          let finalH = parseInt(h) / scaleX;
          console.log('cropImage( img,', x, ',', y, ',', parseInt(w), ',', parseInt(h), ')');
          console.log('cropImage( img,', finalX, ',', finalY, ',', finalW, ',', finalH, ')');
          /*ctx.save();*/
          cropImage(img, finalX/*x / scaleX*/, finalY/*y / scaleY*/, finalW/*parseInt(w) / scaleX*/, finalH/*parseInt(h) / scaleY*/);
          /*ctx.restore();*/
          ...
          // console.log('点击确认截图时rectArr:', rectArr);
          ctx.save();
          ctx.strokeStyle = '#FF0000';
          ctx.rect(x, y, parseInt(w), parseInt(h));
          ctx.stroke();
          ctx.restore();
        });
  ...
  if (!graph.isEnabled()) {
    cutBtn.className = 'geButton mxDisabled';
  }
  ldiv.appendChild(cutBtn);
  ...
  function download() {
    console.log('缩放比例', scaleX);
    let scaleY = 842 / myCanvas.width;
    myCanvas.width = 842;
    myCanvas.height = scaleY * myCanvas.height;
    // ctx.clearRect(0, 0, myCanvas.width, myCanvas.height);  // 清空canvas画布
    ctx.drawImage(img, 0, 0, myCanvas.width, myCanvas.height);  // 将图片加入到canvas中
    // console.log('打印时rectArr:', rectArr);
    console.log(myCanvas.width, myCanvas.height);
    ctx.save();
    ctx.strokeStyle = '#FF0000';
    rectArr.forEach(function (el, index, array) {
      ctx.rect(el.rectX * scaleY, el.rectY * scaleY, el.rectW * scaleY, el.rectH * scaleY);
      ctx.stroke();
    });
    ctx.restore();
    // rectArr = [];
    ...
    ctx.strokeStyle = '#FF0000';
    rectArr.forEach(function (el, index, array) {
      ctx.rect(el.rectX  * scaleX, el.rectY  * scaleX, el.rectW  * scaleX, el.rectH  * scaleX);
      ctx.stroke();
    });
    ctx.restore();
  }
  ...
  // 导出图片按钮
  let outputBtn = link.cloneNode();
  outputBtn.innerHTML = '<div class="geSprite geSprite-outdent" style="display:inline-block;"></div>';
  outputBtn.setAttribute('title', mxResources.get('outputBtn'));
  mxEvent.addListener(outputBtn, 'click', function (evt) {
    if (graph.isEnabled()) {
      graph.model.beginUpdate();
      try {
        console.log("导出截图...");
  ...
  if (!graph.isEnabled()) {
    confirmBtn.className = 'geButton mxDisabled';
  }
  ldiv.appendChild(outputBtn);


  /*
  * 调整图片大小模块
  * */
  let adjustSize = link.cloneNode();
  adjustSize.innerHTML = '<select id="scalingRatio" name="缩放尺寸">\n' +
    ...
    '</select>';
  adjustSize.setAttribute('title', mxResources.get('adjustSize'));
  if (Graph.fileSupport) {
    mxEvent.addListener(adjustSize, 'change', function (evt) {
      if (graph.isEnabled()) {
        graph.model.beginUpdate();
        try {
          var vs = $('#scalingRatio  option:selected').val();
          scaleX = parseInt(vs) / 100;
          console.log('scalingRatio=>', vs);
          console.log('缩放比例:', scaleX * 100, '%');
          rectArr = [];
          // console.log('清空rectArr:', rectArr);
          /*ctx.clearRect(0, 0, myCanvas.width, myCanvas.height);  // 清空canvas画布*/
    ...
    if (!graph.isEnabled()) {
      adjustSize.className = 'geButton mxDisabled';
    }
    ldiv.appendChild(adjustSize);  // 将按钮作为底部工具栏的子元素加入
  }
  ...
  function cropImage(img, cropPosX, cropPosY, width, height) {
    ...
    newCtx.drawImage(img, cropPosX, cropPosY, width, height, 0, 0, width, height);
    console.log('开始剪切的 (x, y) 坐标:(', cropPosX, ',', cropPosY, ')');
    console.log('被剪切图像的尺寸（宽x高）=', width, 'x', height);
    console.log('截图在画布上放置的(x, y)起始坐标:(', 0, ',', 0, ')');
    console.log('画布上放置的截图尺寸（宽x高）=', width, 'x', height);
    ...
    // 将截图绘制到画布上
    let test = newImage.src;
    editorUi.loadImage(test, mxUtils.bind(this, function (img) {
      editorUi.resizeImage(img, test, mxUtils.bind(this, function (data2, w2, h2) {
        editorUi.showImageDialog('title', test, function (newValue, w, h) {
            ...
            if (newValue != null && (newValue.length > 0 || cells.length > 0)) {
              ...
              try {
                ...
                cells = [graph.insertVertex(graph.getDefaultParent(), null, '', pt.x, pt.y, w, h,
                  'myImage;shape=myImage;imageAspect=0;aspect=fixed;verticalLabelPosition=bottom;verticalAlign=top;connectable=0;')];
                select = cells;
                ...
                if (style[mxConstants.STYLE_SHAPE] != 'myImage' && style[mxConstants.STYLE_SHAPE] != 'label') {
                  /*console.log('88888');*/
                  graph.setCellStyles(mxConstants.STYLE_SHAPE, 'myImage', cells);
        ...
        barrier(index, mxUtils.bind(this, function () {
          ...
          if (data2 != null/* && data2.length < editorUi.maxImageBytes*/) { // 取消导入图片大小限制
            var s = (!resizeImages || !editorUi.isResampleImage(test, resampleThreshold)) ? 1 : Math.min(1, Math.min(maxSize / w2, maxSize / h2));
            return fn(data2, 'image/png', x + index * gs, y + index * gs, Math.round(w2 * s), Math.round(h2 * s), 'newImage');
          } /*else {
  ...
  let maxSize = 520; // Defines the maximum size for images.
  let resampleThreshold = 100000; // Images above 100K should be resampled.
  let gs = 10;  // graph.gridSize;

  let fn = function (data, mimeType, x, y, w, h) {
    // console.log('mxgraph/Dialogs中创建了一个ImageDialog弹窗');
    var dlg = new ImageDialog();
    dlg.apply(data);
  };
  let resizeImages = true;
  let count = 1;	// 同时打开的文件数量
  let remain = count;
  // let queue = [];
  let barrierFn = function (queue) // barrierFn
  {
    // Invokes elements of queue in order 按顺序调用队列元素
    for (let i = 0; i < queue.length; i++) {
      queue[i]();
    }
  };
  let resultFn = mxUtils.bind(this, function (cells) {
  ...
  this.window = new mxWindow(mxResources.get('cutImageTool'), div, x, y, w, h, true, true, null, null, true);
  this.window.minimumSize = new mxRectangle(0, 0, 210, 297); // 最小尺寸
  this.window.destroyOnClose = false;
  this.window.setMaximizable(false);
  this.window.setResizable(true);
  this.window.setClosable(true);
  ...
  this.window.addListener(mxEvent.SHOW, mxUtils.bind(this, function () {
    /*console.log('mxEvent.SHOW执行');*/
    this.window.fit();
  }));

  this.window.setLocation = function (x, y) {
    /*console.log('setLocation()执行');*/
