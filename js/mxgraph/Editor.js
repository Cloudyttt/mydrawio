 ...
 * 指定对话框中确定/取消按钮的顺序。默认值为true。cancel first用于macs，windows/confluence使用cancel last。
 */
...
{
	this.graph.gridEnabled = false;/*node.getAttribute('grid') != '0' && (!this.isChromelessView() || urlParams['grid'] == '1');*/
	this.graph.gridSize = parseFloat(node.getAttribute('gridSize')) || mxGraph.prototype.gridSize;
	this.graph.graphHandler.guidesEnabled = true;//node.getAttribute('guides') != '0'; // 参考线
	this.graph.setTooltips(false);//node.getAttribute('tooltips') != '0'); // 悬浮提示
	this.graph.setConnectable(false);//node.getAttribute('connect') != '0'); // 连接点
	this.graph.connectionArrowsEnabled = false;//node.getAttribute('arrows') != '0'; // 连接箭头
	this.graph.foldingEnabled = false;//node.getAttribute('fold') != '0'; // 折叠
...
PrintDialog.prototype.create = function(editorUi)
{
	...
	{
		console.log('preview in mxgraph/Editor.js');
...
PrintDialog.printPreview = function(preview)
{
	try
	{
		if (preview.wnd != null)
		{
			...
			// google chrome的解决方案，它需要一点延迟，以便在生产中呈现svg contentsNeeds测试
			if (mxClient.IS_GC)
			{
...
PrintDialog.createPrintPreview = function(graph, scale, pf, border, x0, y0, autoOrigin)
{
	...
	// console.log('svgRoot in createPrintPreview 创建预览...', svgRoot);
	...
	preview.writeHead = function(doc)
	{
		...
		doc.writeln('  body > div { padding:0;box-sizing:content-box; }');
		doc.writeln('}');
		doc.writeln('</style>');
	};
	console.log('createPrintPreview()返回值：mxPrintPreview对象', preview);
	return preview;
};
...
{
	return [{key: 'a4', title: 'A4 (210 mm x 297 mm)', format: mxConstants.PAGE_FORMAT_A4_PORTRAIT},];
};
