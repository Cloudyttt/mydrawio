...
(function () {
  ...
  function CubeShape() {
    // CubeShape() <= mxCylinder() <= mxShape() <= initStyle()
    mxCylinder.call(this);	// CubeShape.mxCylinder()
  };
  /*
	* function CubeShape(){
	...
	* }
	* */
  mxUtils.extend(CubeShape, mxCylinder);	// CubeShape继承自mxCylinder
  ...
  CubeShape.prototype.paintVertexShape = function (c, x, y, w, h) {
    ...
    if (!this.outline) {  // 如果绘制的图形不属于轮廓（属于绘制某种现实中的物体）
      c.setShadow(false);
  ...
  mxCellRenderer.registerShape('cube', CubeShape);  // 全局注册Cube图形
  //至此，完成一个自定义图形Cube的创建
  /****************************************************************************************************************/
  var tan30 = Math.tan(mxUtils.toRadians(30));  //将给定的度数30转换为对应弧度Math.PI / 6。
