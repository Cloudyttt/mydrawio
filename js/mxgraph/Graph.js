 ...
 * 构造新的图形实例。
 * 请注意，构造函数不采用容器，因为创建UI需要图形实例，而UI反过来将为图形创建容器。
 * 因此，容器稍后在editorui中分配。
 */
/**
 * Defines graph class.
...
Graph = function (container, model, renderHint, stylesheet, themes) {
  mxGraph.call(this, container, model, renderHint, stylesheet); // this.mxGraph(container, model, renderHint, stylesheet)
  ...
  // Implements a listener for hover and click handling on edges 实现悬停和单击边缘处理的侦听器
  if (this.edgeMode) {
    var start = {
      point: null,
      event: null,
  ...
  if (typeof mxVertexHandler !== 'undefined') {
    ...
    this.selectRegion = function (rect, evt) {
      // console.log('selectRegion执行...');
      var cells = this.getAllCells(rect.x, rect.y, rect.width, rect.height); // 获取所有在矩形框中的图形
      /*console.log('rect.x, rect.y, rect.width, rect.height', rect.x, rect.y, rect.width, rect.height);*/
      this.selectCellsForEvent(cells, evt); // 将cells中的元素设置为"选中"状态
        ...
        if (parent != null) {
          ...
          for (var i = 0; i < childCount; i++) {
            ...
            if (state != null && this.isCellVisible(cell) && mxUtils.getValue(state.style, 'locked', '0') != '1') {
              ...
              let w0 = Math.round(box.width);
              let h0 = Math.round(box.height);

              let flag = true;

              // 情况一: 元素完全在橡胶圈外
              if (x0 > x + width || x0 + w0 < x || y0 + h0 < y || y0 > y + height) {
                flag = false;
              }

              // let flag = box.x >= x && box.y + box.height <= bottom && box.y >= y && box.x + box.width <= right;

              if ((model.isEdge(cell) || model.isVertex(cell)) && flag) {

                /*console.log('x0, y0, w0, h0', x0, y0, w0, h0);
                console.log('x, y, width, height', x, y, width, height);
                console.log('左边超出', x0 + w0 , '<', x);
                console.log('右边超出', x0, '>', x + width);
                console.log('上边超出', y0 + h0, '<', y);
                console.log('下边超出', y0, '>', y + height);
                console.log('box', box);
...
/**
 * Shortcut for capability check. 功能检查的快捷方式。
 */
...
/**
 * Resolves the given cells in the model and selects them. 解析模型中给定的单元格并选择它们。
 */
...
HoverIcons.prototype.init = function () {
  ...
  mxEvent.addListener(this.graph.container, 'mouseleave', mxUtils.bind(this, function (evt) {
    if (false) {
      console.log('HoverIcons-mouseleave');
    }
  ...
  // 检查连接处理程序是否在鼠标中处于活动状态作为插入可能的双连接的解决方案
  ...
  this.graph.addMouseListener(
    {
      mouseDown: mxUtils.bind(this, function (sender, me) {
        /*console.log('HoverIcons-mouseDown');*/
      ...
      mouseMove: mxUtils.bind(this, function (sender, me) {
        /*console.log('HoverIcons-mouseMove');*/
        var evt = me.getEvent();
      ...
      mouseUp: mxUtils.bind(this, function (sender, me) {
        /*console.log('HoverIcons-mouseUp');*/
        var evt = me.getEvent();
...
HoverIcons.prototype.getState = function (state) {
  if (state != null) {
    ...
    if (!this.graph.getModel().contains(cell)) {
      ...
      // Uses connectable parent vertex if child is not connectable	如果子顶点不可连接，则使用可连接父顶点
      ...
      // Ignores locked cells and edges	忽略锁定的单元格和边缘
...
if (typeof mxVertexHandler != 'undefined') {
  (function () {
        ...
        for (var i = 0; i < cells.length; i++) {
          ...
          * */
          if (model.isEdge(cell)) {
            var src = model.getTerminal(cell, true); // 源结点
            var trg = model.getTerminal(cell, false); // 目标结点
            ...
            if (geo != null) {
              ...
              // 以某一点作为中心对称点对图形做中心对称（旋转180度）变换
              var centrosymmetry = function (primiryPoint, originPoint) {
                let x = originPoint.x;
                let y = originPoint.y;
                let tempX = primiryPoint.x;
                let tempY = primiryPoint.y;
                primiryPoint.x = x + x - tempX;
                primiryPoint.y = y + y - tempY;
              };
              ...
              let minX = sp.x <= tp.x ? sp.x : tp.x;
              let maxX = sp.x >= tp.x ? sp.x : tp.x;
              let minY = sp.y <= tp.y ? sp.y : tp.y;
              let maxY = sp.y >= tp.y ? sp.y : tp.y;
              /*console.log(minX, minY, maxX, maxY);*/

              if (geo.points != null) {
                /*console.log('geo.points', geo.points);*/

                geo.points.forEach(function (el, index, array) {  // 遍历所有边结点获得最小/最大的x坐标/y坐标
                  minX = el.x <= minX ? el.x : minX;
                  maxX = el.x >= maxX ? el.x : maxX;
                  minY = el.y <= minY ? el.y : minY;
                  maxY = el.y >= maxY ? el.y : maxY;
                });
                /*console.log(minX, minY, maxX, maxY);*/
                let centerX = minX + Math.abs(maxX - minX) / 2;
                let centerY = minY + Math.abs(maxY - minY) / 2;
                var op = {
                  x: centerX,
                  y: centerY
                };
                /*console.log('op', op);*/
                geo.points.forEach(function (el, index, array) { // 遍历中心对称变化所有边结点
                  centrosymmetry(el, op);
                });
                /*geo.points.reverse();*/
              } else {
                let centerX = minX + Math.abs(maxX - minX) / 2;
              ...
              centrosymmetry(tp, op);
              centrosymmetry(sp, op);

              geo.setTerminalPoint(sp, true);
              geo.setTerminalPoint(tp, false);
    ...
    Graph.prototype.getFreeInsertPoint = function () {
      ...
      // Places at same x-coord and 2 grid sizes below existing graph	在现有图形下放置相同的X坐标和2个网格大小
      var x = 50;/*this.snap(Math.round(Math.max(pt.x, bds.x / view.scale - view.translate.x +
				((bds.width == 0) ? 2 * this.gridSize : 0))));*/
      var y = 150;/*this.snap(Math.round(Math.max(pt.y, (bds.y + bds.height) / view.scale - view.translate.y +
				2 * this.gridSize)));*/
    ...
    Graph.prototype.addText = function (x, y, state) {
      ...
      label.value = '文字';
      label.style = 'text;html=1;resizable=0;points=[];fontSize=16;'
     ...
     * 添加用于单击带有链接的形状的处理程序。这将替换标签中的所有链接。
     */
      ...
      var mouseListener =
        {
          ...
          mouseUp: function (sender, me) {
            ...
            if (linkNode == null && (((Math.abs(this.scrollLeft - graph.container.scrollLeft) < tol && Math.abs(this.scrollTop - graph.container.scrollTop) < tol) &&
    ...
    Graph.prototype.distributeCells = function (horizontal, cells) {
      ...
      if (cells != null && cells.length > 1) {
        ...
        for (var i = 0; i < cells.length; i++) {
          if (this.getModel().isVertex(cells[i])) {
            ...
            if (state != null) {
              ...
              max = (max != null) ? Math.max(max, tmp) : tmp;	// 找到最右边（水平）/最下的（垂直）的元素中心点x坐标（水平）/y坐标（垂直）
              min = (min != null) ? Math.min(min, tmp) : tmp;	// 找到最左边（水平）/最上的（垂直）的元素中心点x坐标（水平）/y坐标（垂直）
        ...
        if (vertices.length > 2)	// 当选中的元素多于两个时
        {
          ...
          this.getModel().beginUpdate();
          try {
            var dt = (max - min) / (vertices.length - 1);	// 感觉最远两个点的距离计算出每个元素的间隔
            var t0 = min;
    ...
    Graph.prototype.getSvg = function (background, scale, border, nocrop, crisp, ignoreSelection, showText, imgExport, linkTarget, hasShadow) {
      ...
      console.log('getSvg in mxgraph/Graph.js');
    ...
    mxCellEditor.prototype.startEditing = function (cell, trigger) {
      ...
      this.textarea.innerHTML = cell.value;
      // console.log('cell======>', cell);
      /*if (fileName !== undefine){
        this.textarea.innerHTML = fileName;
      }*/
      ...
      if (state != null && state.style['html'] == 1) {
        ...
        this.textarea.style.fontSize = 16;
      } else {
    ...
    // 使用ctrl+shift（+alt）拖动（“剪刀工具”）覆盖/扩展Rubberband以进行空间处理
    ...
    mxRubberband.prototype.mouseUp = function (sender, me) {
      ...
      if (this.first != null && this.currentX != null && this.currentY != null) {
        ...
        if (!mxEvent.isAltDown(me.getEvent())) { // Alt键没有按下
      ...
      if (execute) {
        if (mxEvent.isAltDown(me.getEvent()) && this.graph.isToggleEvent(me.getEvent())) {
          ...
          // console.log('情况三');
          var rect = new mxRectangle(this.x, this.y, this.width, this.height); // (x0, y0, dx, dy)
          /*console.log('rect参数==>this.x, this.y, this.width, this.height:', this.x, this.y, this.width, this.height);*/
          // console.log('me.getEvent()', me.getEvent());
          this.graph.selectRegion(rect, me.getEvent()); //框选关键函数
        }
