...
/**
 * Specifies the delay for the tooltip. Default is 16 px. 左侧图形工具提示框中图形缩略图与边界的距离
 */
    ...
    if (this.currentElt != elt) {
      ...
      var show = mxUtils.bind(this, function () {
        ...
        if (this.tooltip == null) {
          this.tooltip = document.createElement('div'); // 创建提示框div容器
          this.tooltip.className = 'geSidebarTooltip';
          this.tooltip.style.zIndex = mxPopupMenu.prototype.zIndex - 1;
          document.body.appendChild(this.tooltip);

          // console.log('this.editorUi.editor.graph.getStylesheet()', this.editorUi.editor.graph.getStylesheet());
          this.graph2 = new Graph(this.tooltip, null, null, this.editorUi.editor.graph.getStylesheet());  // 图形缩略图
        ...
        var width = bounds.width + 2 * this.tooltipBorder + 4;  // 提示框尺寸
...
/**
 * Creates a thumbnail for the given cells.  为给定单元格创建缩略图。
 */
...
/**
 * Creates and returns a new palette item for the given image.为给定图像创建并返回新的调色板项。
 */
Sidebar.prototype.createItem = function (cells, title, showLabel, showTitle, width, height, allowCellsInserted) {
  ...
  // Blocks default click action	阻止默认单击操作
  ...
  this.createThumb(cells, this.thumbWidth, this.thumbHeight, elt, title, showLabel, showTitle, width, height); // 左侧图形栏图形图标
  var bounds = new mxRectangle(0, 0, width, height);

  if (cells.length > 1 || cells[0].vertex) { // 图形
    /*console.log('cells[0]==>', cells[0]);
    console.log('elt', elt);*/
    var ds = this.createDragSource(elt, this.createDropHandler(cells, true, allowCellsInserted, bounds), this.createDragPreview(width, height, cells[0]), cells, bounds);
  ...
  } else if (cells[0] != null && cells[0].edge) {	// 边线
    var ds = this.createDragSource(elt, this.createDropHandler(cells, false, allowCellsInserted, bounds), this.createDragPreview(width, height, cells[0]), cells, bounds);
    this.addClickHandler(elt, ds, cells);
  }
...
/**
 * Creates a drop handler for inserting the given cells. 创建用于插入给定单元格的放置处理程序。
 */
Sidebar.prototype.updateShapes = function (source, targets) {
  ...
  try {
    ...
    // Lists the styles to carry over from the existing shape 列出要从现有形状继承的样式
  ...
  return mxUtils.bind(this, function (graph, evt, target, x, y, force) {
    ...
    if (elt == null && graph.isEnabled()) {
      ...
      if (cells.length > 0) {
        ...
        // Holding alt while mouse is released ignores drop target 释放鼠标时按住alt忽略放置目标
        ...
        if (!graph.isCellLocked(target || graph.getDefaultParent())) {
          ...
          try {
            ...
            // Splits the target edge or inserts into target group 拆分目标边或插入到目标组中
            ...
            // Executes parent layout hooks for position/order 执行位置/顺序的父布局挂钩
            ...
            if (graph.getSelectionCount() == 1) {
              ...
              var selectStyle = mxUtils.getStylename(selectCell.getStyle());
              var clickCell = select[0];
              var clickedStyle = mxUtils.getStylename(clickCell.style);
              // 判断在选中图片的时候点击了放大镜
              if (selectStyle == 'myImage' && clickedStyle == 'magnify') {
                // 初始化放大镜大框
                clickCell.style = 'myMagnifyTarget;shape=myMagnifyTarget;rounded=0;fillColor=none;connectable=0;editable=0;imageAlign=left;imageVerticalAlign=top;imageAspect=1;aspect=fixed;';
                // 这里获取的image参数，里面多了字符";base64,"，其中的分号导致style无法正常工作，所以要去掉
                graph.setCellStyles('image', graph.getCellStyle(selectCell).image.replace(';base64', ''), [clickCell]);

                // 初始化放大镜小框
                var sg = selectCell.geometry;
                var rect = graph.insertVertex(selectCell, null, '', sg.width / 2, sg.height / 2, 30, 30, 'myMagnifySource;rounded=0;fillColor=none;connectable=0;editable=0;');

                // 连接放大镜 大框 和 小框
                graph.insertEdge(null, null, '', rect, clickCell, 'endArrow=none;rounded=0;bendable=0;');
                // 触发更新（位于EditorUi.js中的onCellsChanged）
                graph.fireEvent(new mxEventObject(mxEvent.CELLS_MOVED, 'cells', [rect]));
              }
            }
          } catch (e) {
...
/**
 * Creates and returns a preview element for the given width and height. 创建并返回给定宽度和高度的预览元素。
 */
Sidebar.prototype.createDragPreview = function (width, height, cell) {

  let elt = document.createElement('div');
  elt.style.border = this.dragPreviewBorder;
  elt.style.className = 'geSidebarTooltip';
  elt.style.zIndex = mxPopupMenu.prototype.zIndex - 1;
  elt.style.width = width + 'px';
  elt.style.height = height + 'px';
  let graph0 = new Graph(elt, null, null, this.editorUi.editor.graph.getStylesheet());  // 图形缩略图
  // console.log('this.editorUi.editor.graph.getStylesheet()', this.editorUi.editor.graph.getStylesheet());
  graph0.resetViewOnRootChange = false;
  graph0.foldingEnabled = false;
  graph0.gridEnabled = false;
  graph0.autoScroll = false;
  graph0.setTooltips(false);
  graph0.setConnectable(false);
  graph0.setEnabled(false);
  graph0.addCells(cell);
  if (!mxClient.IS_SVG) {
    this.graph2.view.canvas.style.position = 'relative';
  }
...
/**
 * Creates a drag source for the given element. 为给定元素创建拖动源
 */
Sidebar.prototype.createDragSource = function (elt, dropHandler, preview, cells, bounds) {
  /*console.log('createDragSource...');*/
  // Checks if the cells contain any vertices 检查单元格是否包含任何顶点
  ...
  var sidebar = this;
  for (var i = 0; i < cells.length; i++) { 	// cells.length = 1
  ...
  var dragSource = mxUtils.makeDraggable(elt, this.editorUi.editor.graph, mxUtils.bind(this, function (graph, evt, target, x, y) {
    // console.log('dragSource执行...');
    if (this.updateThread != null) {
      window.clearTimeout(this.updateThread);
    }

    if (cells != null && currentStyleTarget != null && activeArrow == styleTarget) { // false
    ...
    } else if (cells != null && activeArrow != null && currentTargetState != null && activeArrow != styleTarget) { // false
      ...
      dropHandler.apply(this, arguments);	// this.dropHandler(elt, dropHandler, preview, cells, bounds)
    }
  ...
  dragSource.dragOver = function (graph, evt) {
    ...
    mxDragSource.prototype.dragOver.apply(this, arguments); // this.dragOver(argument)
...
/**
 * Adds a handler for inserting the cell with a single click. 单击事件监听器
 */
...
Sidebar.prototype.addClickHandler = function (elt, ds, cells) {
  ...
  ds.mouseUp = function (evt) {
    ...
    oldMouseUp.apply(ds, arguments); // ds.oldMouseUp(evt)
    mxUtils.setOpacity(elt, 100);
    first = null;
...
Sidebar.prototype.createVertexTemplateEntry = function (style, width, height, value, title, showLabel, showTitle, tags) {
  tags = (tags != null && tags.length > 0) ? tags : title.toLowerCase();	// 用传递进来的参数tags或者参数title做标签
...
/**
 * Creates a drop handler for inserting the given cells.创建放置处理程序，用于插入给定单元格。
 */
