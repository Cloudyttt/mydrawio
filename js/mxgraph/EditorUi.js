...
EditorUi = function(editor, container, lightbox)
{
	...
	// Adds tooltip when mouse is over scrollbars to show space-drag panning option 当鼠标悬停在滚动条上时添加工具提示以显示空间拖动平移选项
	mxEvent.addListener(this.diagramContainer, 'mousemove', mxUtils.bind(this, function(evt)
	{
		...
		{
			this.diagramContainer.setAttribute('title', mxResources.get('panTooltip')); // 按住空格键并拖拽以移动画布
		}
		else
		{
	...
	// Cell 的移动或尺寸变化事件监听
	this.onCellsChanged = mxUtils.bind(graph, function (sender, evt) {
		// 判断是否是放大镜的小框发生了变化
        var sourceCell = evt.getProperty('cells')[0];
        var sourceStyle = mxUtils.getStylename(sourceCell.getStyle());
        if (sourceStyle == 'myMagnifySource') {
            /*console.log(sourceCell);*/
            if (!sourceCell.edges || !sourceCell.edges[0] || !sourceCell.edges[0].target || !sourceCell.parent.geometry) return;
            var targetCell = [sourceCell.edges[0].target]; // 获取放大镜大框
            var sg = sourceCell.geometry;  // 放大镜小框位置尺寸
            var pg = sourceCell.parent.geometry; // 放大镜小框所在的图片的位置尺寸

			// 计算放大镜大框：上下左右需要裁剪的比例
            this.setCellStyles('clipTop', sg.y/pg.height, targetCell);
            this.setCellStyles('clipLeft', sg.x/pg.width, targetCell);
            this.setCellStyles('clipBottom', (pg.height-sg.y-sg.height)/pg.height, targetCell);
            this.setCellStyles('clipRight', (pg.width-sg.x-sg.width)/pg.width, targetCell);

            // 更新放大镜大框尺寸
            var tg = targetCell[0].geometry;
            tg.height = tg.width*sg.height/sg.width;
        }

    })

    graph.addListener(mxEvent.CELLS_MOVED, this.onCellsChanged);
    graph.addListener(mxEvent.CELLS_RESIZED, this.onCellsChanged);
...
EditorUi.prototype.updateActionStates = function()
{
	...
	var actions = ['cut', 'copy', 'bold', 'italic', 'underline', 'delete', 'duplicate',
	               ...
	               'edit', 'toFront', 'toBack', /*'multiply', */'lockUnlock', 'solid', 'dashed', 'pasteSize',
...
/**
 * Displays a print dialog.显示打印对话框。
 */
...
{
	console.log('在mxgraph的Editor.js中创建了一个showImageDialog');
...
EditorUi.prototype.createKeyHandler = function(editor)
{
	...
	if (!this.editor.chromeless || this.editor.editable)
	{
		...
		keyHandler.bindAction(74, true, 'cutImageTool', true); // Ctrl+Shift+J
