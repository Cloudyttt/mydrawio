...
Toolbar.prototype.init = function()
{
	...
	{
		this.addSeparator();
		var elts = this.addItems(['zoomIn', 'zoomOut', 'cutImageTool']);
		...
		elts[2].setAttribute('title', mxResources.get('cutImageTool') + ' (' + this.editorUi.actions.get('cutImageTool').shortcut + ')');	// 截图
	}
	...
	if (sw >= 550)
	{
		this.addItems(['-', 'toFront', 'toBack'/*, 'multiply'*/]);	// 新增正片叠底
	}
	...
	var insertMenu = this.addMenu('', mxResources.get('insert') + ' (' + mxResources.get('insertImage'/*'doubleClickTooltip'*/) + '或文字)', true, 'insert', null, true);
