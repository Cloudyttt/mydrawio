...
Actions.prototype.init = function () {
  ...
  this.addAction('lockUnlock', function () {
    if (!graph.isSelectionEmpty()) {
      ...
      try {
        ...
        graph.toggleCellStyles('connectable', false);
      } finally {
        graph.getModel().endUpdate();
      }
    }
  ...
  /*// 正片叠底
  this.addAction('multiply', function () {
    /!*console.log('正片叠底...');*!/
    graph.makeMultiply();
    /!*
  ...
  // 正片叠底
  this.addAction('grayscale', function () {
    /!*console.log('正片叠底...');*!/
    graph.makeMultiply();
    /!*
    var isMultiply = mxUtils.getValue(this.style, 'isMultiply', 0);
  ...
  /*this.addAction('cutImageTool', function() { graph.orderCells(true); }, null, null);*/
  /*
  * 截图工具事件处理函数
  * */
  this.addAction('cutImageTool', mxUtils.bind(this, function () {
    if (this.ImageWindow == null) {
      // LATER: Check outline window for initial placement
      this.ImageWindow = new ImageWindow(ui, document.body.offsetWidth - 600, 120, 600, 600);
    } else {
      this.ImageWindow.window.setVisible(!this.ImageWindow.window.isVisible()); // 关闭截图工具
    }
  ...
  this.addAction('image...', function () {
    if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
      ...
      ui.showImageDialog(title, value, function (newValue, w, h, filename) {  // myapp/Dialog.js中ImageDialog的参数fn
        // Inserts image into HTML text
        /*console.log('mxgraph/Action ==> showImageDialog获取到的文件名:', filename);*/
        ...
        if (graph.cellEditor.isContentEditing()) {
          ...
          if (newValue != null && (newValue.length > 0 || cells.length > 0)) {
            ...
            graph.getModel().beginUpdate();
            try {
              // Inserts new cell if no cell is selected 如果未选择单元格，则插入新单元格

              /*console.log('cells长度为0');*/
              var pt = graph.getFreeInsertPoint();
              cells = [graph.insertVertex(graph.getDefaultParent(), null, filename, 50/*pt.x*/, 150/*pt.y*/, w, h,
                'myImage;shape=myImage;imageAspect=0;aspect=fixed;verticalLabelPosition=bottom;verticalAlign=top;connectable=0;fontSize=16;')];
              select = cells;
              ...
              // Sets shape only if not already shape with image (label or image) 仅当尚未使用图像（标签或图像）进行形状设置时才设置形状
              var state = graph.view.getState(cells[0]);
              var style = (state != null) ? state.style : graph.getCellStyle(cells[0]);
      ...
      }, graph.cellEditor.isContentEditing()/*false*/, !graph.cellEditor.isContentEditing()/*true*/);
    }
  }).isEnabled = isGraphEnabled;
