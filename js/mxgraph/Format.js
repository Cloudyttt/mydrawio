...
/**
 * Returns information about the current selection. 返回有关当前所选内容的信息。
 */
...
Format.prototype.updateSelectionStateForCell = function (result, cell, cells) {
  ...
  if (state != null) {
    ...
    result.isMultiply = result.isMultiply && this.isMultiplyState(state);
    result.grayscale = result.grayscale && this.isGrayscaleState(state);
    /*console.log('result.isMultiply: ', result.isMultiply);*/
    /*console.log('result.grayscale: ', result.grayscale);*/
...
Format.prototype.isShadowState = function (state) {
  var shape = mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null);
  return (shape != 'image');  // 当选中的形状不是image类型时返回true
};

Format.prototype.isMultiplyState = function (state) {
  var shape = mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null);
  /*console.log('shape: ', shape);*/
  return (shape == 'myImage');  // 当选中的形状是myImage类型时返回true
};

Format.prototype.isGrayscaleState = function (state) {
  var shape = mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null);
  /*console.log('shape: ', shape);*/
  return (shape == 'myImage');  // 当选中的形状是myImage类型时返回true
};
...
ArrangePanel.prototype.addLayerOps = function (div) {
  ...
  /*return div;*/
  /*// 最顶部目录栏添加截图工具按钮
  var btn = mxUtils.button(mxResources.get('cutImageTool'), function (evt) {
    ui.actions.get('cutImageTool').funct();
  });

  btn.setAttribute('title', mxResources.get('cutImageTool') + ' (' + this.editorUi.actions.get('cutImageTool').shortcut + ')');
  btn.style.width = '100px';
  div.appendChild(btn);*/
...
ArrangePanel.prototype.addGeometry = function (container) {
  ...
  var width = this.addUnitInput(div, 'pt', 84, 44, function () {
    widthUpdate.apply(this, arguments); // this.widthUpdate(arguments)
  });
  ...
  myDiv.style.flexDirection = 'flex-row';
  myDiv.style.height = '50px';
  myDiv.style.justifyContent = 'flex-end';
  myDiv.style.alignItems = 'center';
  myDiv.style.paddingRight = '20px';
  // myDiv.style.marginBottom = '10px';


  let myTitle = document.createElement('div');
  myTitle.innerHTML = `<span id="mySize"></span>`;

  let link = document.createElement('a');
  link.className = 'geButton';
  // link.style.position = 'absolute'
  // link.style.textAlign = 'relative'
  ...
  adjustSize.innerHTML =
    `<select id="selectSize" name="缩放尺寸">
      <option id="asd" value="整体缩放">整体缩放</option>
      <option value="130 pt">6</option>
      <option value="160 pt">5</option>
      <option value="200 pt">4</option>
      <option value="270 pt">3</option>
      <option value="400 pt">2</option>
    </select>`;
  myDiv.appendChild(myTitle);
  myDiv.appendChild(adjustSize);
  ...
  if (Graph.fileSupport) {
    mxEvent.addListener(adjustSize, 'change', function (evt) {
      if (graph.isEnabled()) {
        ...
        try {
          let vs = $('#selectSize  option:selected').val();
          // let label = document.getElementById("mySize");
          // $('#mySize').text('asd');
          // $('#selectSize').val(vs);
          height.value = vs;
          heightUpdate(evt);
...
StyleFormatPanel.prototype.init = function () {
  ...
  if (!ss.containsLabel) {
    ...
    this.container.appendChild(this.addStroke(this.createPanel())); // 线型
    ...
    /*// 灰度
    var grayscalePanel = this.createRelativeOption(mxResources.get('grayscale'), mxConstants.STYLE_GRAYSCALE, 41);
    grayscalePanel.style.paddingTop = '8px';
    grayscalePanel.style.paddingBottom = '8px';
    this.container.appendChild(grayscalePanel);*/
...
/**
 * Adds the label menu items to the given menu and parent. 将标签菜单项添加到给定的菜单和父级。
 */
...
StyleFormatPanel.prototype.addEffects = function (div) {
  ...
  var listener = mxUtils.bind(this, function (sender, evt, force) {
    ...
    if (!ss.containsImage) {
      addOption(mxResources.get('multiply'), 'isMultiply', 0);
    }
    if (!ss.containsImage) {
      addOption(mxResources.get('grayscale'), 'grayscale', 0);
    }
    if (ss.glass) {
