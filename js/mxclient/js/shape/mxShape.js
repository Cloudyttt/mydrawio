 ...
 * 所有形状的基类。MXgraph中的形状是SVG、VML和HTML的单独实现。
 * 要使用的实现由创建形状时从<mxCellRenderer>中分配的<Dialect>属性控制。
 * 必须为形状指定方言，并且它通常取决于浏览器和图形的配置（请参见<mxgraph>rendering提示）。
 *
 ...
 * the graph.
 * 对于SVG和VML中支持的每个形状，MXGRAPH中都存在相应的形状，即文本、图像、矩形、菱形、椭圆和多段线。
 * 其他形状是这些形状（如标签和泳道）的组合，或者由一个或多个（填充的）路径对象（如演员和圆柱体）组成。
 * HTML实现是可选的，但对于图的仅HTML视图可能是必需的。
 *
 * Custom Shapes:
 *
 ...
 * respectively.
 * 要从该类扩展，基本代码如下所示。在自定义形状仅由一个填充区域或一个填充区域和一个附加笔划组成的特殊情况下，应分别对<mxactor>和<mxCylinder>进行子类化。
 *
 * (code)
 ...
 * 要在现有图形实例中注册自定义形状，必须在图形的单元格呈现器中以新名称注册该形状，如下所示：
 * (code)
 ...
 * 为了使用该形状，您可以在样式表中引用上面的给定名称。例如，要更改默认顶点样式的形状，将使用以下代码：
 * (code)
...
{
	this.stencil = stencil;	// stencil: mxShape对象属性，用于保留自定义形状的<mxstencil>。
	this.initStyles(); 	// 将一些列样式设置为其默认值（初始化元素样式）。
};
 ...
 * 保留要绘制形状的方言。这可以是<mxconstants>中的格式常量之一。
 */
mxShape.prototype.dialect = null; // svg / html / xml
 ...
 * Variable: stencil
 *
 * Holds the <mxStencil> that defines the shape.保留定义形状的<mxstencil>。
 */
 ...
 * SVG笔划的事件容忍度（在PX）中。默认值为8。只有当<pointerevents>为真时，才会传递到画布中。
 */
 ...
 * 指定是否应处理模具外部的指针事件。默认值为假。为向后兼容1.x分支设置为true。
 */
 ...
 * calling <apply>.
 * 指定形状是否应绘制为轮廓。这将禁用所有填充颜色，并可用于禁用不应为轮廓绘制的其他绘图状态。
 * 默认值为假。这应该在调用<apply>之前设置。
 */
...
mxShape.prototype.redraw = function()
{
	...
	if (this.visible && this.checkBounds()) // true
	{
		// console.log('情况一');
		this.node.style.visibility = 'visible';
		this.clear();

		if (this.node.nodeName == 'DIV' && (this.isHtmlAllowed() || !mxClient.IS_VML)) // false
		{
...
mxShape.prototype.configureCanvas = function(c, x, y, w, h)
{
	...
	console.log(c);
	console.log(c.state);*/
	c.setGrayscale(this.grayscale / 100);
 ...
 * 这将保留对<style>的引用。
 * 如果需要保留对单元格的引用，可以重写此方法并存储对state.cell或<mxcellstate>本身的本地引用。
 * 如果<outline>应为true，请确保在调用此方法之前设置它。
 *
 * Parameters:
 *
...
mxShape.prototype.apply = function(state)
{
	...
	if (this.style != null)
	{
		...
		this.grayscale = mxUtils.getValue(this.style, mxConstants.STYLE_GRAYSCALE, this.grayscale);
