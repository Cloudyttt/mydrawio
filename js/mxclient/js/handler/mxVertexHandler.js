...
mxVertexHandler.prototype.mouseDown = function (sender, me) {
  ...
  if (!me.isConsumed() && this.graph.isEnabled() && (tol > 0 || me.getState() == this.state)) {
    ...
    if (handle != null) {
      this.start(me.getGraphX(), me.getGraphY(), handle); // 传入鼠标点击位置
      // console.log('me.getGraphX()', me.getGraphX());
      // console.log('me.getGraphY()', me.getGraphY());
      me.consume();
    }
  }
};
...
mxVertexHandler.prototype.start = function (x, y, index) {
  ...
  if (!this.livePreview || this.isLivePreviewBorder()) { // 单个图形为false
...
mxVertexHandler.prototype.mouseMove = function (sender, me) {
  if (!me.isConsumed() && this.index != null) {
    ...
    if (!this.inTolerance) {
      if (this.index <= mxEvent.CUSTOM_HANDLE) {
        ...
        this.moveLabel(me);
      } else if (this.index == mxEvent.ROTATION_HANDLE) { // 旋转
        // console.log('rotateVertex');
        this.rotateVertex(me);
      } else { // 重绘尺寸
        // console.log('resizeVertex');
        // console.log('me', me);
  ...
  else if (!this.graph.isMouseDown && this.getHandleForEvent(me) != null) { // => false
    me.consume(false);
  }
};
...
mxVertexHandler.prototype.resizeVertex = function (me) {
  var ct = new mxPoint(this.state.getCenterX(), this.state.getCenterY()); // 中心点
  var alpha = mxUtils.toRadians(this.state.style[mxConstants.STYLE_ROTATION] || '0'); // => 0
  var point = new mxPoint(me.getGraphX(), me.getGraphY()); // 鼠标位置
  var tr = this.graph.view.translate; // 页面偏移
  var scale = this.graph.view.scale; // 默认 => 1
  var cos = Math.cos(-alpha); // cos(0) => 1
  var sin = Math.sin(-alpha); // sin(0) => 0
  ...
  var geo = this.graph.getCellGeometry(this.state.cell); // mxGeometry
  ...
  // console.log(this.unscaledBounds);
...
mxVertexHandler.prototype.updateLivePreview = function (me) {
  ...
  var scale = this.graph.view.scale; // => 1
  var tr = this.graph.view.translate; // 页面偏移

  // Saves current state
  var tempState = this.state.clone(); // mxCellState
  /*----------------------------------------图形宽高和位置根据外边框实时调整---------------------------------------------*/
  // Temporarily changes size and origin 临时更改大小和原点
  ...
  // Needed to force update of text bounds 需要强制更新文本边界
  this.state.unscaledWidth = null;

  // Redraws cell and handles 重画单元格和句柄
  var off = this.state.absoluteOffset;
  off = new mxPoint(off.x, off.y);

  // Required to store and reset absolute offset for updating label position 需要存储和重置绝对偏移以更新标签位置
  ...
  if (geo != null) {
    ...
    if (offset != null && !geo.relative) { // => false
  ...
  // console.log('this.state', this.state);
  this.state.view.graph.cellRenderer.redraw(this.state, true);  // 实时绘制关键函数

  // Redraws connected edges TODO: Include child edges
  this.state.view.invalidate(this.state.cell);
  this.state.invalid = false;
  this.state.view.validate();
  this.redrawHandles(); // 实时绘制拖拽点
 ...
 */
mxVertexHandler.prototype.redrawHandles = function () {
  // console.log('mxVertexHandler.redrawHandles');
  var tol = this.tolerance;
  // console.log('this.tolerance', this.tolerance);
  ...
  if (this.sizers != null && this.sizers.length > 0 && this.sizers[0] != null) {
    if (this.index == null && this.manageSizers && this.sizers.length >= 8) {
      ...
      if (this.horizontalOffset != 0 || this.verticalOffset != 0) {
        ...
        s.x = s.x - this.horizontalOffset / 2;
        s.width = s.width + this.horizontalOffset;
        s.y = s.y - this.verticalOffset / 2;
        s.height = s.height + this.verticalOffset;
      }
      // console.log(this.sizers);
      if (this.sizers.length >= 8) // 过小时隐藏四个角上的拖拽点
      {
    ...
    var r = s.x + s.width; // 最右边值
    var b = s.y + s.height; // // 最下边值

    if (this.singleSizer) {
      // console.log('singleSizer');
      this.moveSizerTo(this.sizers[0], r, b);
    } else {
      var cx = s.x + s.width / 2; // 中心坐标
      var cy = s.y + s.height / 2;

      if (this.sizers.length >= 8) {
        var crs = ['nw-resize', 'n-resize', 'ne-resize', 'e-resize', 'se-resize', 's-resize', 'sw-resize', 'w-resize'];

        var alpha = mxUtils.toRadians(this.state.style[mxConstants.STYLE_ROTATION] || '0'); // 弧度 =>0
        var cos = Math.cos(alpha);
        var sin = Math.sin(alpha);
        // console.log('alpha', alpha);
        ...
        var ct = new mxPoint(s.getCenterX(), s.getCenterY()); // 中心点
        var pt = mxUtils.getRotatedPoint(new mxPoint(s.x, s.y), cos, sin, ct); // 左上角起始点
        // console.log('ct', ct);
        // console.log('pt', pt);
        // console.log('this.sizers[0]', this.sizers[0]);
        this.moveSizerTo(this.sizers[0], pt.x, pt.y);
        this.sizers[0].setCursor(crs[mxUtils.mod(0 + da, crs.length)]);
        // console.log(crs[mxUtils.mod(0 + da, crs.length)]);
  ...
  if (this.rotationShape != null) // => true
  {
