 ...
 * 扩展<mxEventSource>以实现图形模型。
 * 图形模型作为一个包装器，围绕负责存储实际图形数据结构的单元。
 * 该模型充当一个事务包装器，其中包含所有更改的事件通知，而单元则包含用于更新实际数据结构的原子操作。
 * Layers:
 * 
 ...
 * if required.
 * 使用<mxtternalchange>设置给定<mxcell>的源或目标终端，并将更改添加到当前事务。
 * 如果需要，此实现将使用<updatedgeparent>更新边缘的父级。
 * Parameters:
 * 
 * edge - <mxCell> that specifies the edge.
 ...
 * 返回一个数组中层次结构的最上面的单元格，该数组不包含其所包含的每个<mxcell>的子代。应删除单元格数组中的重复项以提高性能。
 * 
 * Parameters:
 * 
 ...
 * <endUpdate>.
 * 将<updateLevel>增加一个。使用<endupdate>将事件通知排队，直到<updateLevel>达到0。
 *
