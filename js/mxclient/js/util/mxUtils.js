...
var mxUtils =
  {
     ...
     * 返回一个包装函数，该函数将给定函数的执行范围锁定到指定范围的。在函数内部，“this”关键字成为对该范围的引用。
     */
    bind: function (scope, funct) {
      return function () {
        return funct.apply(scope, arguments);	// scope.funct(argument)
      };
    },
     ...
     * the given default value if the value is null.返回给定关联数组中给定键的值，如果该值为空，则返回给定默认值。
     *
     * Parameters:
     *
     * array - Associative array that contains the value for the key.包含键值的关联数组。
     * key - Key whose value should be returned.
     * defaultValue - Value to be returned if the value for the given
     * key is null.
     */
    // getValue(array, key, defaultValue)：返回array[key]的值
     ...
     * Parameters:
     *
     * ctor - Constructor of the subclass.子类构造函数
     * superCtor - Constructor of the superclass.父类构造函数
     */
    extend: function (ctor, superCtor) {
      var f = function () {
      };	// 创建临时构造函数
      f.prototype = superCtor.prototype;	// 将superCtor的构造函数赋值给f的构造函数

      ctor.prototype = new f();	// 使得子类的原型对象__proto__指向父类的原型对象，从而实现继承原型方法

      ctor.prototype.constructor = ctor;	// // 使得子类的constructor指针重新指向子类的构造函数

    },
    /*
	* var extend = function(ctor, superCtor) {
	*
	* }
	*
	* */
     ...
     * 使用指定容器的偏移量转换指定的点（x，y），并返回一个新的结果。
     * (code)
     ...
     * 返回n除以m的余数。
     * 您应该使用它而不是内置操作，因为内置操作不能正确处理负数。
     */
    mod: function (n, m)	// 取n除以m的余数
    {
      return ((n % m) + m) % m;
    },
     ...
     * 将给定的dom元素配置为充当指定图形的拖动源。
     * 返回一个新的<mxdragsource>。
     * 如果启用了<mxdragsource.guideenabled>，则必须在函数中使用x和y参数来匹配预览位置。
     *
     * Example:
     *
     * (code)
