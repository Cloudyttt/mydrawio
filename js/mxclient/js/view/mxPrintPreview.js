...
function mxPrintPreview(graph, scale, pageFormat, border, x0, y0, borderColor, title, pageSelector) {
  console.log('mxPrintPreview执行');
  // let svgRoot = graph.getSvg(graph.background, null, null, null, null, true);
  // console.log('svgRoot in mxPrintPreview', svgRoot);
 ...
 * 指定是否应根据实际图表内容的左上角自动计算原点。所需的偏移量将添加到<open>中的<x0>和<y0>。默认值为true。
 */
...
mxPrintPreview.prototype.appendGraph = function (graph, scale, x0, y0, forcePageBreaks, keepOpen) {
  console.log('mxPrintPreview.prototype.appendGraph 执行...');
  this.graph = graph;
  this.scale = (scale != null) ? scale : 1 / graph.pageScale;
  this.x0 = x0;
  this.y0 = y0;
...
mxPrintPreview.prototype.open = function (css, targetWindow, forcePageBreaks, keepOpen) {
  ...
  console.log('mxPrintPreview.prototype.open 执行...');
  ...
  try {
    ...
    for (var i = 0; i < vpages; i++) {
      ...
      for (var j = 0; j < hpages; j++) {
        ...
        console.log('div in open', typeof div);
        // var myDefs = `<defs xmlns="http://www.w3.org/2000/svg">
        ...
        //           </filter>
        //         </defs>`;
        // var mySvg = div.getElementsByTagName('svg');
        // mySvg.list.insertBefore(mySvg, div.childNodes[0]);
        // console.log('div 2 in open', div);
 ...
 * 在给定的文档中写入头部分，而不使用开头和结尾的头标记。
 */
mxPrintPreview.prototype.writeHead = function (doc, css) {
  console.log('mxPrintPreview.prototype.writeHead 执行...');
  if (this.title != null) {
 ...
 * content - Callback that adds the HTML content to the inner div of a page. 将HTML内容添加到页面内部div的回调。
...
mxPrintPreview.prototype.renderPage = function (w, h, dx, dy, content, pageNumber) {
  ...
  try {
    ...
    if (dx != 0 || dy != 0) { // false
      ...
      console.log('viewport0000000000000000000000', viewport);
  ...
  content(arg); // 将svg作为子结点添加到innerDiv中国
...
mxPrintPreview.prototype.addGraphFragment = function (dx, dy, scale, pageNumber, div, clip) {
  ...
  try {
    ...
    if (mxClient.IS_IE) {
      ...
      var svgDoc = div.firstChild.ownerDocument;
      ...
      filter2.setAttribute('id', 'grayscale');
      ...
      let defs = div.firstChild.getElementsByTagName('defs');
      let defsElt = null;
      defsElt = (svgDoc.createElementNS != null) ? svgDoc.createElementNS(mxConstants.NS_SVG, 'defs') : svgDoc.createElement('defs');
      div.firstChild.insertBefore(defsElt, div.firstChild.firstChild);
      console.log('div.firstChild =====>', div.firstChild);
      console.log('div.defsElt =====>', defsElt);
      ...
      // let myDefs = `<defs xmlns="http://www.w3.org/2000/svg"><filter id="dropShadow"><feGaussianBlur in="SourceAlpha" stdDeviation="0" result="blur"/><feOffset in="blur" dx="0" dy="0" result="offsetBlur"/><feFlood flood-color="#3D4574" flood-opacity="0.4" result="offsetColor"/><feComposite in="offsetColor" in2="offsetBlur" operator="in" result="offsetBlur"/><feBlend in="SourceGraphic" in2="offsetBlur"/></filter><filter id="grayscale" x="0" y="0"><feColorMatrix type="matrix" values="0.3333 0.3333 0.3333 0 0 0.3333 0.3333 0.3333 0 0 0.3333 0.3333 0.3333 0 0 0 0 0 1 0"/></filter></defs>`;
      // div.firstChild.insertBefore(myDefs,div.firstChild.childNodes[0]);
