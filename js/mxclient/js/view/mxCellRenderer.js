 ...
 * 静态数组，其中包含该类的所有实例都知道的全局注册形状。要添加新形状，应使用static<mxcellenderer.registershape>函数。
 */
 ...
 * of the renderer.在此容器实例的指定键下注册给定的构造函数。
 *
 * Example:
 *
 * (code)
 ...
 * updating all cells.
 * 更新给定单元格状态形状的边界或点和比例。
 * 这是在mxsGraphView.validatePoints中调用的，作为更新所有单元格的最后一步。
 * Parameters:
 *
...
mxCellRenderer.prototype.redrawShape = function (state, force, rendering) {
  ...
  // Forces creation of new shape if shape style has changed 如果形状样式已更改，则强制创建新形状
  if (state.shape != null && state.shape.style != null && state.style != null && state.shape.style[mxConstants.STYLE_SHAPE] != state.style[mxConstants.STYLE_SHAPE]) { // false
    console.log('如果形状样式已更改，则强制创建新形状');
    state.shape.destroy();
    state.shape = null;
  }

  if (state.shape == null && state.view.graph.container != null && state.cell != state.view.currentRoot && (model.isVertex(state.cell) || model.isEdge(state.cell))) { // false
    // console.log('情况一');
  ...
  } else if (!force && state.shape != null && (!mxUtils.equalEntries(state.shape.style, state.style) || this.checkPlaceholderStyles(state))) { // false
    // console.log('情况二');
  ...
  if (state.shape != null) {
    ...
    // defined as the bounds are updated for the given points inside the shape
		// 如果需要，重新绘制单元格；如果定义了点，则忽略对边界的更改，因为为形状内的给定点更新了边界。
    if (force || this.isShapeInvalid(state, state.shape)) {
      ...
      if (rendering == null || rendering) { // true
				// console.log('333');
