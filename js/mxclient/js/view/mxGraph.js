...
{
  // console.log('mxGraph.prototype.setCellStyles');
  cells = cells || this.getSelectionCells();
 ...
 * Group: Order
 */
mxGraph.prototype.makeMultiply = function(cells){

  if (cells == null)
  {
    cells = mxUtils.sortCells(this.getSelectionCells(), true);
  }
  console.log('cells[0] in mxGraph.js', cells[0]);
  this.model.beginUpdate();
  try
  {
    console.log('正片叠底');
   /* applyStyleVal('isMultiply', '1', {name: 'isMultiply', dispName: '正片叠底', type: 'bool', deVal: 0});*/
  }
  finally
  {
    this.model.endUpdate();
...
{
  let isIncludeMyImage = false;
  console.log('mxGraph.groupCells');
  if (cells == null) // 没有cells传人，所有选中元素作为
	{
		cells = mxUtils.sortCells(this.getSelectionCells(), true);
	}
  for(let i = 0; i < cells.length; i++){

    if(cells[i].style.indexOf('myImage') !== -1 || cells[i].style.indexOf('image') !== -1){ // 当选中的形状是myImage类型时返回true
      // console.log(cells[i]);
      isIncludeMyImage = true;
    }
  }
  ...
  cells = this.getCellsForGroup(cells);
	...
	if (group == null) // 没有group传入，新创建一个group
	{
 ...
 * 通过在前一个源和给定单元格之间添加newEdge并将给定边缘的源重新连接到给定单元格来拆分给定边缘。
 * 此方法在事务正在进行时激发<mxEvent.SPLIT_EDGE>。
 * 返回插入的新边缘。
 * Parameters:
 *
  ...
  dx = dx || 0;
	...
	try
	{
		if (newEdge == null)
		{
			...
			// Removes waypoints before/after new cell 删除新单元格前后的航路点
			var state = this.view.getState(edge);
 ...
 * 按给定的向量移动指定的单元格，使用disconnectgraph断开单元格连接为disconnect为true。
 * 当事务正在进行时，此方法激发<mxevent.cells_moved>。
 * 按给定的向量移动指定的单元格，使用disconnectgraph断开单元格连接为disconnect为true。
 * 此方法在事务正在进行时激发<mxevent.cells_moved>。
 */
	...
	if (cells != null && (dx != 0 || dy != 0))
	{
		...
		try
		{
			...
			for (var i = 0; i < cells.length; i++)
			{
				...
				if (extend && this.isExtendParent(cells[i])) // false
				{
					this.extendParent(cells[i]);
				}
				else if (constrain) // false
				{
					this.constrainChild(cells[i]);
				}
			}
 ...
 * 如果给定事件是切换事件，则返回true。
 * 如果在Mac OS上按下meta key（cmd）或在任何其他平台上按下control，则此实现返回true。
 */
 ...
 * 返回给定矩形中包含的给定父级的子顶点和边。
 * 结果将添加到可选的结果数组中，该数组将返回。
 * 如果未指定结果数组，则创建并返回新数组。
 *
 * Parameters:
 *
 ...
 * parent - <mxCell> that should be used as the root of the recursion（递归）.
 * Default is current root of the view or the root of the model.
 * result - Optional array to store the result in.
 */
mxGraph.prototype.getCells = function(x, y, width, height, parent, result)
{
  console.log('getCells执行...');
 ...
 *
 * Returns the first cell from the array of selected <mxCells>. 返回所选<mxcells>数组中的第一个单元。
 */
 ...
 * replacing the selection depending on whether the given mouse event is a
 * toggle event.
 * 根据给定的鼠标事件是否为切换事件，通过将给定的单元格添加到选定内容或替换选定内容来选择给定的单元格。
 * Parameters:
 *
 * cells - Array of <mxCells> to be selected.
 * evt - Optional mouseevent that triggered the selection.
...
{
	if (this.isToggleEvent(evt)) // false
	{
    console.log('isToggleEvent');
