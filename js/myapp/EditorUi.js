...
(function () {
   ...
   * Defines the maximum size for images.
   */
  EditorUi.prototype.maxImageSize = 4000;/*520;*/

  /**
   * Images above 100MB should be resampled.
   */
  EditorUi.prototype.resampleThreshold = 100000000;/*100000;*/

  /**
   * Maximum allowed size for images is 100 MB.
   */
  EditorUi.prototype.maxImageBytes = 100000000;/*1000000;*/
  ...
  /**
   * Returns true if no external comms allowed or possible 如果不允许或可能没有外部命令，则返回true
   */
  ...
  /**
   * Translates this point by the given vector. 将此点转换为给定的向量。
   *
  ...
  EditorUi.prototype.createSpinner = function (x, y, size) {
    ...
    var spinner = new Spinner({
      ...
      color: (uiTheme == 'dark') ? '#ff0000' : '#000000', // #rgb or #rrggbb
    ...
    // Extends spin method to include an optional label 扩展spin方法以包括可选标签
    var oldSpin = spinner.spin;

    spinner.spin = function (container, label) {
      var result = false;
      if (!this.active) {
        /*console.log('spin => 不活跃');*/
        oldSpin.call(this, container);  // this.oldSpin(container)
        this.active = true;

        if (label != null) {
          /*console.log('spin => label不为空');*/
          var status = document.createElement('div');
          status.style.position = 'absolute';
          status.style.whiteSpace = 'nowrap';
          status.style.background = '#ff0000'/*'#4B4243'*/;
          ...
          status.style.left = 0 /*Math.max(0, x) + 'px'*/;
          status.style.top = 0 /*Math.max(0, y + 70) + 'px'*/;
    ...
    // Extends stop method to remove the optional label 扩展停止方法以删除可选标签
  ...
  EditorUi.prototype.getFileData = function (forceXml, forceSvg, forceHtml, embeddedCallback, ignoreSelection, currentPage, node, compact, file) {
    ...
    if (this.pages != null && this.currentPage != this.pages[0] && (forceSvg || (!forceXml && file != null && /(\.svg)$/i.test(file.getTitle())))) {
  ...
  EditorUi.prototype.downloadFile = function (format, nonCompressed, addShadow, ignoreSelection, currentPage, pageVisible, transparent) {
    try {
      console.log('downloadFile...', format);
      ...
      if (format == 'xml') {
        console.log('case xml');
        var data = '<?xml version="1.0" encoding="UTF-8"?>\n' + ((nonCompressed) ? mxUtils.getXml(this.editor.getGraphXml(ignoreSelection)) : this.getFileData(true, null, null, null, ignoreSelection, currentPage));
        ...
        if (format == 'svg') {
          ...
          console.log('111',this.editor.graph);
          // if (addShadow) {
        ...
        if (format == 'xmlpng') {
          console.log('case xmlpng');
          filename = basename + '.png';
        } else if (format == 'jpeg') {
          console.log('case jpeg');
  ...
  EditorUi.prototype.createDownloadRequest = function (filename, format, ignoreSelection, base64, transparent, currentPage) {
    ...
    // 仅为不包含文件数据的图像导出当前页面，但对于包含XML或包含所有页面的PDF的其他格式，我们需要发送完整的数据，并使用“从/到URL”参数指定要导出的页面。
    ...
    if (file != null) {
      try {
        ...
        if (!this.isOffline() && file.getMode() != null) {
          EditorUi.logEvent({
            category: 'File', action: 'open', label:
...
//				}));
   ...
   */
  EditorUi.prototype.showImageDialog = function (title, value, fn, ignoreExisting/*false*/, convertDataUri/*true*/) {
    // KNOWN: IE+FF don't return keyboard focus after image dialog (calling focus doesn't help)
    /*console.log('myapp/EditorUi中创建了一个ImageDialog...');*/
  ...
  EditorUi.prototype.saveCanvas = function (canvas, xml, format) {
    ...
    // console.log(canvas, xml, format);
    var ext = ((format == 'jpeg') ? 'jpg' : format);
    var filename = this.getBaseFilename() + '.' + ext;
    var data = this.createImageDataUri(canvas, xml, format); // 已经被压缩
...
//		}
   ...
   * 最后3个参数是可选的，只有在数据可以按原样存储在客户机端而不需要服务器往返时才能使用。
   * @param {number} dx X-coordinate of the translation.
   * @param {number} dy Y-coordinate of the translation.
   */
        ...
        if (mode == '_blank' || newTitle != null && newTitle.length > 0) {
          ...
          if (xhr != null) {
            if (mode == App.MODE_DEVICE || mode == 'download' || mode == '_blank') {
              console.log('case 1 in save Qequest');
              xhr.simulate(document, '_blank');
            } else {
              console.log('case 2 in save Qequest');
              this.pickFolder(mode, mxUtils.bind(this, function (folderId) {
                mimeType = (mimeType != null) ? mimeType : ((format == 'pdf') ? 'application/pdf' : 'image/' + format);
...
//		}
  ...
  EditorUi.prototype.exportImage = function (scale, transparentBackground, ignoreSelection, addShadow, editable, border, noCrop, currentPage, format) {
    ...
    // console.log('第一步，调用exportImage函数!!!');
      ...
      try {
        this.exportToCanvas(mxUtils.bind(this, function (canvas) {
          ...
          try {
            ...
            this.saveCanvas(canvas, (editable) ? this.getFileData(true, null, null, null, ignoreSelection, currentPage) : null, format);
          } catch (e) {
          ...
          this.spinner.stop();
          this.handleError(e);
        }), null, ignoreSelection, scale || 1, transparentBackground, addShadow, null, null, border, noCrop);
      } catch (e) {
        this.spinner.stop();
        this.handleError(e);
      }
   ...
   *
   */
  EditorUi.prototype.exportToCanvas = function (callback, width, imageCache, background, error, limitHeight, ignoreSelection, scale, transparentBackground, addShadow, converter, graph, border, noCrop) {
    // console.log('第二部: exportToCanvas导出为canvas');
    ...
    // console.log('asdasd', graph.getSvg(bg, null, null, noCrop, null, ignoreSelection, null, null, null, addShadow)); 返回的svg没有defs
    this.convertImages(graph.getSvg(bg, null, null, noCrop, null, ignoreSelection, null, null, null, addShadow),
      ...
      try {
        console.log('this.editor.graph', this.editor.graph);
        this.editor.graph.addSvgShadow(svgRoot); // 添加defs
        // if (addShadow) {
        //   this.editor.graph.addSvgShadow(svgRoot);
        // }
        var done = mxUtils.bind(this, function () {
          ...
          this.convertMath(graph, svgRoot, true, mxUtils.bind(this, function () {
            ...
            // console.log(img.src); // 这里的image已经没有defs
          }));
        });
  ...
  EditorUi.prototype.convertImages = function (svgRoot, callback, imageCache, converter) {
    ...
    var convertImages = mxUtils.bind(this, function (tagName, srcAttr) {
      ...
      console.log('svgRoot', svgRoot);
      console.log('svgRoot', svgRoot.toString());
  ...
  /**
   * Imports the given XML into the existing diagram. 将给定的XML导入到现有关系图中。
   */
  EditorUi.prototype.convertDataUri = function (uri) {
    ...
    // 处理数据URI的特殊情况，需要重写该情况才能在单元格样式中使用，以删除分号
    if (uri.substring(0, 5) == 'data:') {
      var semi = uri.indexOf(';');

      if (semi > 0) {
        uri = uri.substring(0, semi)/*'data:image/png'*/ + uri.substring(uri.indexOf(',', semi + 1)/**/);
      }
    }
   ...
   * Imports the given XML into the existing diagram.
   */
  EditorUi.prototype.importFile = function (data, mimeType, dx, dy, w, h, filename, done, file, crop, ignoreEmbeddedXml/*false*/) {
    /*var editorUi = Menu.editorUi;
    var graph = editorUi.editor.graph;*/
    /*startEditing();*/
    console.log('正在执行importFile()...');
    console.log('data', data);
    ...
    if (mimeType.substring(0, 5) == 'image') {  // true
      /*console.log('image');*/
      var containsModel = false;

      if (mimeType.substring(0, 9) == 'image/png') {  // true
      ...
      if (!containsModel) { // true
        /*console.log('!containsModel');*/
        var graph = this.editor.graph;

        // Strips encoding bit (eg. ;base64,) for cell style, 即convertDataUrl
        ...
        if (crop && graph.isGridEnabled()) {
          /*console.log('crop && graph.isGridEnabled()');*/
          dx = graph.snap(dx);
          dy = graph.snap(dy);
        }

        cells = [graph.insertVertex(null, null, filename, dx, dy, w, h,
          'myImage;shape=myImage;connectable=0;clipPath=inset(0% 0% 0% 0%);verticalLabelPosition=bottom;verticalAlign=top;imageAspect=1;aspect=fixed;connectable=0;fontSize=16;image=' + data + ';')];
      }
   ...
   * 导入文件
   */
  // this.importFiles(evt.dataTransfer.files, x, y, this.maxImageSize, null, null, null, null, mxEvent.isControlDown(evt)/*resizeDialogg*/, null, null, mxEvent.isShiftDown(evt));
  EditorUi.prototype.importFiles = function (files, x, y, maxSize, fn, resultFn, filterFn, barrierFn, resizeDialog, maxBytes/*->null*/, resampleThreshold/*压缩闸值->null*/, ignoreEmbeddedXml/*忽略嵌入的XML->null*/) {
    console.log('myapp/importFiles()...');
    x = (x != null) ? x : 0;  // 0
    y = (y != null) ? y : 0;  // 0
    maxSize = (maxSize != null) ? maxSize : this.maxImageSize;  // editorUi.maxImageSize
    maxBytes = (maxBytes != null) ? maxBytes : this.maxImageBytes;  // editorUi.maxImageBytes

    var crop = (x != null) && (y != null);  // true
    var resizeImages = false;

    // Checks if large images are imported
    var largeImages = false;
    if (!mxClient.IS_CHROMEAPP && files != null)	// 判读图片是否为largeImages
    {
      var thresh = resampleThreshold || this.resampleThreshold; // 压缩图片闸值

      for (var i = 0; i < files.length; i++) // 	遍历Filelist
      {
        if (files[i].type.substring(0, 6) == 'image/' && files[i].size > thresh)	// type:"image/png"
        {
          largeImages = true;
          break;
        }
    ...
    var doImportFiles = mxUtils.bind(this, function () {
      var graph = this.editor.graph;
      var gs = graph.gridSize;  // 10
      /*console.log('this.editor.graph:', graph);*/
      // fn = ImageDialog.apply(data)
      ...
      resultFn = (resultFn != null) ? resultFn : mxUtils.bind(this, function (cells) {
        console.log('resultFn == null');
        graph.setSelectionCells(cells);
      });

      if (this.spinner.spin(document.body, mxResources.get('loading'))) {
        /* console.log('选中并打开的files:', files);*/

        var count = files.length;	// 同时打开的文件数量
        var remain = count;
        var queue = [];

        // Barrier waits for all files to be loaded asynchronously 屏障等待异步加载所有文件
        ...
        for (let i = 0; i < count; i++) {
          ...
          (mxUtils.bind(this, function (index) {
            ...
            if (file != null) {
              ...
              /*console.log('当前操作的图片:', file);*/
              reader.onload = mxUtils.bind(this, function (e) {
                /*console.log('+++++++++++++++开始第', index + 1, '次reader.onload+++++++++++++++');*/
                /*console.log('file.name:', file.name);
                console.log('reader:', reader);*/
                if (filterFn == null || filterFn(file)) {
                  if (file.type.substring(0, 6) == 'image/') {
                    // Checks if SVG contains content attribute
                    if (file.type.substring(0, 9) == 'image/svg') { // -> false 不执行
                      console.log('图片格式为svg的情况...');
                      ...
                      // Checks if PNG+XML is available to bypass code below 检查PNG+XML是否可以绕过下面的代码
                      let containsModel = false;
                      /*if (file.type === 'image/png') { // true
                        console.log('图片格式:image/png ');
                        let xml = (ignoreEmbeddedXml) ? null : this.extractGraphModelFromPng(e.target.result); // e.target.result为打开图片base64编码数据, 此处xml = null
                        if (xml != null && xml.length > 0) {  // xml == null 不执行
                          console.log('此处xml != null');
                          var img = new Image();
                          img.src = e.target.result;
                          barrier(index, mxUtils.bind(this, function () {
                            return fn(xml, 'text/xml', 0/!*x + index * gs*!/, 0/!*y + index * gs*!/,
                      ...
                      // Additional asynchronous step for finding image size 用于查找图像大小的其他异步步骤
                      if (!containsModel) {
                        /*console.log('不包含模型');*/
                        // Cannot load local files in Chrome App 无法在Chrome应用程序中加载本地文件
                        if (mxClient.IS_CHROMEAPP) {  // false, 不执行
                          ...
                          /*console.log('不在Chrome应用程序中加载');*/
                          this.loadImage(e.target.result, mxUtils.bind(this, function (img) { // onload: success
                            this.resizeImage(img, e.target.result, mxUtils.bind(this, function (data2, w2, h2) {  // data2 = e.target.result / w2, h2即为调整尺寸后的图片
                              barrier(index, mxUtils.bind(this, function () {
                                ...
                                if (data2 != null/* && data2.length < maxBytes*/) {
                                  ...
                                  var s = 1;
                                  /*console.log('正在插入的图片名称: ', file.name);*/
                                  /*console.log('apply传递给importFile作为其参数的图片base64编码（data2）', data2);*/
                                  var filename = file.name.substring(0, file.name.indexOf('.'));
                                  console.log('图片名称:', file.name);
                                  let mimeType = file.type;
                                  return fn(data2, mimeType, x + index * gs, y + index * gs, Math.round(w2 * s), Math.round(h2 * s), filename); // ImageDialg.apply(data2);
                                } /*else {
                          ...
                          }), mxUtils.bind(this, function () {  // onload: error
                    ...
                    /*console.log('不执行');*/
                    fn(e.target.result, file.type, 0/*x + index * gs*/, 0/* y + index * gs*/, 240, 160, file.name, function (cells) {
  ...
  /**
   * Resizes the given image if <maxImageBytes> is not null. 如果<maximagebytes>不为空，则调整给定图像的大小。
   */
  ...
  /**
   * Adds the given text to the compressed or non-compressed text chunk. 将给定文本添加到压缩或非压缩文本块。
   */
  ...
  /**
   * Extracts the XML from the compressed or non-compressed text chunk. 从压缩或非压缩文本块中提取XML。
   */
  EditorUi.prototype.extractGraphModelFromPng = function (data) { // 从PNG提取图形模型
    var result = null;
  ...
  EditorUi.prototype.init = function () {
    ...
    // Holds the x-coordinate of the point  保持点的X坐标
    ...
    if (Graph.fileSupport && (!this.editor.chromeless || this.editor.editable)) {
      ...
      // dragover - 当某被拖动的对象在另一对象容器范围内悬浮拖动时触发此事件
      mxEvent.addListener(graph.container, 'dragover', mxUtils.bind(this, function (evt) {
        // console.log('dragover!!!');
        // IE 10 does not implement pointer-events so it can't have a drop highlight
        if (dropElt == null && (!mxClient.IS_IE || document.documentMode > 10)) {
          dropElt = this.highlightElement(graph.container); // 容器高亮
        }
      ...
      mxEvent.addListener(graph.container, 'drop', mxUtils.bind(this, function (evt) {
        ...
        if (graph.isEnabled()) {
          ...
          if (evt.dataTransfer.files.length > 0) {  // 拖拽导入图片的关键处理函数
            console.log('拖动导入的图片:');
            console.log(evt.dataTransfer.files);
            this.importFiles(evt.dataTransfer.files, x, y, this.maxImageSize, null, null, null, null,
              mxEvent.isControlDown(evt)/*resizeDialogg*/, null, null, mxEvent.isShiftDown(evt));
          } else {
  ...
  /**
   * Adds a file drop handler for opening local files.右侧工具栏
   */
