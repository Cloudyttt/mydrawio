...
(function () {
  ...
  Menus.prototype.init = function () {
    ...
    //     editorUi.showDialog(new PrintDialog(editorUi, mxResources.get('formatPdf')).container, 360, (editorUi.pages != null && editorUi.pages.length > 1) ? 420 : 360, true, true);
    //     /*} else {
    //       console.log('在线');
    //       var noPages = editorUi.pages == null || editorUi.pages.length <= 1;
    //       var div = document.createElement('div');
    ...
    //         console.log('case-1 in menu.js');
    //         var allPages = editorUi.addRadiobox(div, 'pages', mxResources.get('allPages'), true); // 所有页面
    //         var currentPage = editorUi.addRadiobox(div, 'pages', mxResources.get('currentPage', null, 'Current Page'), false); // 仅当前页面
    //         var selection = editorUi.addRadiobox(div, 'pages', mxResources.get('selectionOnly'), false, graph.isSelectionEmpty()); // 仅所选内容
    //         var crop = editorUi.addCheckbox(div, mxResources.get('crop'), false, true); // 单页导出
    //
      ...
      if (editorUi.isExportToCanvas()) {
        editorUi.showExportDialog(mxResources.get('image'), false, mxResources.get('export'),
          ...
          mxUtils.bind(this, function (scale, transparentBackground, ignoreSelection,
            ...
            if (!isNaN(val) && val > 0) {
              editorUi.exportImage(val / 20, transparentBackground, ignoreSelection,
    ...
    editorUi.actions.put('exportJpg', new Action(mxResources.get('formatJpg') + '...', function () {
      /*if (editorUi.isExportToCanvas()) {*/
				console.log('导出为canvas');
        editorUi.showExportDialog(mxResources.get('image'), false, mxResources.get('export'), '',
          mxUtils.bind(this, function (scale, transparentBackground, ignoreSelection, addShadow, editable, embedImages, border, cropImage, currentPage) {
            var val = parseInt(scale);
            console.log('scale: ', scale);
            if (!isNaN(val) && val > 0) {
							console.log('case-1 in exportJpg');
							// console.log(val);
							editorUi.exportImage(val / 20, false, ignoreSelection, addShadow, false, border, !cropImage, false, 'jpeg'); // 放大5倍
            }
          }), true, false, 'jpeg');
      /*} else if (!editorUi.isOffline() && (!mxClient.IS_IOS || !navigator.standalone)) {
				console.log('在线');
				editorUi.showRemoteExportDialog(mxResources.get('export'), null, mxUtils.bind(this, function (ignoreSelection, editable) {
					console.log('case-2 in exportJpg');
              ...
              if (newValue.length > 0) {
                try {
                  console.log(JSON.stringify(editorUi.diffPages(editorUi.pages,
                    editorUi.getPagesForNode(mxUtils.parseXml(newValue).documentElement)), null, 2));
    ...
    editorUi.actions.put('insertText', new Action(mxResources.get('text'), function () {
      if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
        graph.startEditingAtCell(insertVertex('文本', 40, 20, 'text;html=1;resizable=0;autosize=1;' +
          'align=center;verticalAlign=middle;points=[];fillColor=none;strokeColor=none;rounded=0;fontSize=16;'));
      }
    ...
    this.put('insert', new Menu(mxUtils.bind(this, function (menu, parent) {
      this.addMenuItems(menu, [/*'insertRectangle', 'insertEllipse', 'insertRhombus', '-',*/
        'insertImage', 'insertText'/*, 'insertLink', '-', */], parent);
    ...
    this.put('view', new Menu(mxUtils.bind(this, function (menu, parent) {
      this.addMenuItems(menu, ((this.editorUi.format != null) ? ['formatPanel'] : []).concat(['outline', 'layers']).concat((editorUi.commentsSupported()) ?
        ['comments', '-'] : ['-']));
