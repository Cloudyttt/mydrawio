...
/**
 * Implementation of the undoable page rename.  可撤消页重命名的实现。
 */
 ...
 *
 * Action to change the current root in a view.更改视图中当前根目录的操作。
 *
    ...
    if (graph.isEnabled() && tmp >= 0) {
      ...
      try {
        ...
        if (next == page && this.pages.length > 1) {
          ...
          console.log('myapp/Page.js/removePage/767行');
...
EditorUi.prototype.duplicatePage = function (page, name) {
  ...
  try {
    ...
    if (graph.isEnabled()) {
      ...
      console.log('myapp/Page.js/duplicatePage/814行');
    ...
    var menu = new mxPopupMenu(mxUtils.bind(this, function (menu, parent) {
      ...
      if (this.editor.graph.isEnabled()) {
        ...
        var item = menu.addItem(mxResources.get('insertPage'), null, mxUtils.bind(this, function () {
          console.log('myapp/Page.js/1101行');
          this.insertPage();
        }), parent);
...
EditorUi.prototype.createPageInsertTab = function () {
  ...
  mxEvent.addListener(tab, 'click', mxUtils.bind(this, function (evt) {
    this.insertPage();
    /*console.log('myapp/createPageInsertTab/click');*/
    mxEvent.consume(evt);
  }));
 ...
 * Returns true if the given string contains an mxfile.
 */
EditorUi.prototype.createTabForPage = function (page, tabWidth, hoverEnabled) { // 设置插入新页面时的页面名称
  /*console.log('myapp/createTabForPage');*/
  var tab = this.createTab(hoverEnabled);
  var name = page.getName() || mxResources.get('untitled');
  var id = page.getId();
  tab.setAttribute('title', name + ((id != null) ? ' (' + id + ')' : ''));
  /*console.log('myapp/createTabForPage/tab.title:', tab.title);*/
...
EditorUi.prototype.createPageMenu = function (page, label) {
  return mxUtils.bind(this, function (menu, parent) {
    ...
    menu.addItem(mxResources.get('insert'), null, mxUtils.bind(this, function () {  // 插入
      console.log('myapp/Pages.js/createPageMenu/1257');
      this.insertPage(null, mxUtils.indexOf(this.pages, page) + 1);
    }), parent);

    menu.addItem(mxResources.get('delete'), null, mxUtils.bind(this, function () {  // 删除
      this.removePage(page);
    }), parent);

    menu.addItem(mxResources.get('rename'), null, mxUtils.bind(this, function () {  // 重命名
      this.renamePage(page, label);
    }), parent);

    menu.addSeparator(parent);

    menu.addItem(mxResources.get('duplicate'), null, mxUtils.bind(this, function () { // 复制
      this.duplicatePage(page, mxResources.get('copyOf', [page.getName()]));
    }), parent);
    console.log('myapp/Page.js/2');
  });
};
